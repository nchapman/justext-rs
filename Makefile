...
CDYLIB  := $(RELEASE_DIR)/libjustext_uniffi.$(CDYLIB_EXT)
...
SWIFT_SRC_DIR  := $(SWIFT_TEST_DIR)/Sources/justext_uniffiFFI

.PHONY: build-swift
build-swift: $(GENERATED_DIR)/swift cargo-build
	$(call require,swift)
	cp $(GENERATED_DIR)/swift/justext_uniffiFFI.h $(SWIFT_SRC_DIR)/
	cp $(GENERATED_DIR)/swift/justext_uniffiFFI.modulemap $(SWIFT_SRC_DIR)/module.modulemap
	mkdir -p $(SWIFT_TEST_DIR)/Sources/Justext
	cp $(GENERATED_DIR)/swift/justext_uniffi.swift \
		$(SWIFT_TEST_DIR)/Sources/Justext/justext_uniffi.swift
